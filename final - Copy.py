...
def create_s3_folder(bucket_name):
    try:
        ...
        folder_name = today.strftime('%d%m')
...
async def main():
    ...
    check_and_update_template('ORISSA', ['odiya_template.docx', 'orissa_template.docx', 'orisa_template.docx'])
    
    check_and_update_template('ANDHRA PRADESH', ['telugu_template.docx', 'telgue_template.docx'])
...
#     running_sample_code = False

#     while True:
#         run_sample_code = input("Do you want to run the sample code? (yes/no): ").strip().lower()
#         if run_sample_code in ["yes", "no"]:
#             running_sample_code = (run_sample_code == "yes")
#             break
#         else:
#             logging.info("Invalid input. Please enter 'yes' or 'no'.")

#     logging.info(f"Upload status: {running_sample_code}")
#     logging.info(f"Compressing PDFs status: {running_sample_code}")
#     logging.info(f"Deleting OUTPUT directory status: {running_sample_code}")

#     if running_sample_code == True:
#             create_new_folder = input("Do you want to create a new folder? (yes/no): ").strip().lower()

#             if create_new_folder == "yes":
#                 S3_URI = create_s3_folder(S3_BUCKET_NAME)
#                 logging.info(f"Creating new folder: {S3_URI}")
#             else:
#                 existing_folder_name = input("Enter the existing folder name: ").strip()
#                 S3_URI = existing_folder_name
#                 logging.info(f"Uploading to existing folder: {S3_URI}")

         
#     state_counts_total = df['State'].value_counts().to_dict()
#     logging.info("\nTotal records for each state:")
#     for state, count in sorted(state_counts_total.items(), key=lambda x: x[0]):
#         logging.info(f"{state}: {count}")
#     logging.info("\n")
...
#     # Creates S3 folder based on today's date
#     # S3_URI = create_s3_folder(S3_BUCKET_NAME)
#     # S3_URI = "0805"
...
#     # # Gets total count of files uploaded in particular folders
    # get_count_of_files_in_bucket(AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY, AWS_REGION, S3_BUCKET_NAME, S3_URI)
